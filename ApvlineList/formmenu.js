...
{//미결 혹은 재기안으로 문서를 열 경우 의견이 있는 경우 의견 창 display 20080919
    ...
    //if (elmList.length > 0) {
    if (elmList.length > 0 && m_oFormEditor.gCommentCnt > 0) {  //위구문 주석처리후 추가 (2012-11-23 HIW)
...
function initBtn(){//debugger;
    ...
    switch(getInfo("loct")){
		...
		case "REJECT":
		    ...
		    if ((getInfo("loct") == "COMPLETE" || getInfo("loct") == "REJECT") && getInfo("INITIATOR_ID") == getInfo("usid")) document.getElementById("btReUse").style.display = "";
		    if (getInfo("fmusyn") == "1") { document.getElementById("btReUse").style.display = "none"; }
		...
		if(getInfo("mode")=="APPROVAL" ||getInfo("mode")=="PCONSULT" || getInfo("mode")=="RECAPPROVAL"  || getInfo("mode")=="SUBAPPROVAL"  ){ //기안부서결재선 및 수신부서 결재선
			...
			if(oFirstNode != null){
				...
				*/
				...
				document.getElementsByName("APVLIST")[0].value = (!window.ActiveXObject) ?(new XMLSerializer()).serializeToString(oResult) : oResult.xml;
...
function getApvList(){
	...
	return (!window.ActiveXObject) ?(new XMLSerializer()).serializeToString(xmlApv) : xmlApv.xml;
}
...
function receiveHTTP(){
	if(m_xmlHTTP.readyState==4){
		...
		if(((!window.ActiveXObject) ?(new XMLSerializer()).serializeToString(xmlReturn) :xmlReturn.xml)==""){
			bCompensate=true;
			sParam=null;			
		}else{
    	...
    	try{
		    ...
		    try{
			    if(m_bFileSave == true){					
                    ...
                    //debugger;
					//for (i = 0; i < m_evalXML.documentElement.parentNode.getElementsByTagName("taskinfo").length; i++) {
					    //if (m_evalXML.documentElement.parentNode.getElementsByTagName("taskinfo")[i].getAttribute("rejectee") == "y")
					       ////removeComment = m_evalXML.documentElement.parentNode.getElementsByTagName("comment")[i].xml;
					       //removeComment = m_evalXML.documentElement.parentNode.getElementsByTagName("taskinfo")[i].childNodes[0].xml
                    //}
					//if (getDefaultXML().indexOf("<comment relatedresult=\"rejectedto\"") > -1) {
					   // defaultXML = getDefaultXML().replace(removeComment,"");
				//	}
		...
		switch(obj.id){
	        ...
	        case "btPrint": //인쇄
	            ...
	            if (m_oFormEditor.document.getElementById("tblDraftBody") == null) {  //에디터없는 양식
	                ...
	                printDiv = m_oFormEditor.document.getElementById("divBodyContent").innerHTML + m_oFormEditor.document.getElementById("divApprovalLine").innerHTML;
			...
			if(commentFlag == true)
			{
				printDiv = m_oFormEditor.document.getElementById("divBodyContent").innerHTML + m_oFormEditor.document.getElementById("divApprovalLine").innerHTML + m_oFormEditor.document.getElementById("tbCommentView").innerHTML;	
			}
			else
			{
				printDiv = m_oFormEditor.document.getElementById("divBodyContent").innerHTML + m_oFormEditor.document.getElementById("divApprovalLine").innerHTML;
...
function getListXML(){
	...
	if (strListVal == null || strListVal == "") {
	    ...
	    if (m_oFormEditor.document.getElementsByName("ATTACH_FILE_INFO")[0].value.indexOf("</fileinfos>") < 0) {
	        return (!window.ActiveXObject) ?(new XMLSerializer()).serializeToString(ofileinfo) :ofileinfo.xml;
	    } else {
	        return "<fileinfos>" + ((!window.ActiveXObject) ?(new XMLSerializer()).serializeToString(ofileinfo) :ofileinfo.xml) + "</fileinfos>";
	    }
	}    
}
...
function insertToList(oList){
	if(m_sAddList == 'receive'){
		...
		aRecDept[document.getElementById("iBody").selectedIndex] = (!window.ActiveXObject) ?(new XMLSerializer()).serializeToString(oList) :oList.xml;
		var sRecDept = "";
		...
		m_oRequestList.loadXML("<?xml version='1.0' encoding='utf-8'?>"+(!window.ActiveXObject) ?(new XMLSerializer()).serializeToString(oList) :oList.xml);
		...
		if(errorNode!=null){
			...
			switch(getInfo("mode")){
				...
				case "COMPLETE" :
					...
					m_oFormEditor.document.getElementsByName("ATTACH_FILE_INFO")[0].value = (!window.ActiveXObject) ?(new XMLSerializer()).serializeToString(xmlReturn.selectSingleNode("response/fileinfos")) : xmlReturn.selectSingleNode("response/fileinfos").xml;
					m_bFileSave = true;
					break;
				case "RECAPPROVAL":
				case "PCONSULT":
				...
				case "REDRAFT":	
					m_bFrmClientFileDitry = true;
					if(xmlReturn.selectNodes("response/fileinfos").length > 0)	m_oFormEditor.document.getElementsByName("ATTACH_FILE_INFO")[0].value =(!window.ActiveXObject) ?(new XMLSerializer()).serializeToString( xmlReturn.selectSingleNode("response/fileinfos")) :  xmlReturn.selectSingleNode("response/fileinfos").xml;
					m_bFileSave = true;
					break;
				    ...
				    if(getInfo("mode") == 'DRAFT' || getInfo("mode") == "TEMPSAVE" || getInfo("mode") == "REDRAFT"){
					    ...
					    if(!oApvList.loadXML(document.getElementsByName("APVLIST")[0].value)){
						    ...
						    if (getInfo("mode") == "REDRAFT") {
							    ...
							    document.getElementsByName("APVLIST")[0].value = (!window.ActiveXObject) ?(new XMLSerializer()).serializeToString(oApvList.documentElement) : oApvList.documentElement.xml;
							    var nodesAllItems;
...
//									    document.getElementById("btDeptDraft").style.display = "inline";
							    ...
							    document.getElementsByName("APVLIST")[0].value =(!window.ActiveXObject) ?(new XMLSerializer()).serializeToString(oApvList) : oApvList.xml;
								    ...
								    if(nodesAllItems.length > 0){
									    ...
									    if(oCheckSteps){					
										    for(var x=0; x<nodesAllItems.length; x++){
										        ...
										        if (enodeItem == null) enodeItem = nodesAllItems[x];
...
function receiveGeneralQuery(){
	if(m_xmlHTTP.readyState==4){
		m_xmlHTTP.onreadystatechange=event_noop;
		if(((!window.ActiveXObject) ?(new XMLSerializer()).serializeToString(m_xmlHTTP.responseXML) :m_xmlHTTP.responseXML.xml)==""){
			//alert(m_xmlHTTP.responseText);
		}else{
			var errorNode=m_xmlHTTP.responseXML.selectSingleNode("response/error");
			if(errorNode!=null){
...
function setWellKnownMailboxUrl(oHTTP, listener) {
	...
	g_xmlHTTP.send((!window.ActiveXObject) ?(new XMLSerializer()).serializeToString(g_oXML) :g_oXML.xml);
}
...
//                                }
          ...
          m_objXML.loadXML((!window.ActiveXObject) ?(new XMLSerializer()).serializeToString(m_oParent):m_oParent.xml);        
          insertToList4ApvLine(oApvList, oCurrentOUNode, m_objXML);
          */
      }
      //특정 금액
  ...
  if(getInfo("mode") == "DRAFT" || getInfo("mode") == "TEMPSAVE"){
      if(getInfo("scCDTApvLine") == "1" && getInfo("scCDTApvLineV") != ""){
	    ...
	    var oCurrentOUNode =oApvList.documentElement.selectSingleNode("division[taskinfo/@status='inactive' or taskinfo/@status='pendig' ]");	
          ...
          parent.menu.document.getElementsByName("APVLIST")[0].value  = (!window.ActiveXObject) ?(new XMLSerializer()).serializeToString(oApvList.documentElement):oApvList.documentElement.xml; 
