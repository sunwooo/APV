<%@ Page Language="C#" AutoEventWireup="true" validateRequest="false" CodeFile="request_svcd.aspx.cs" Inherits="Approval_Forms_request_svcd" %>
...
<body>
    <form id="form1" action="Form.aspx?fmid=<%=fmid %>&scid=<%=scid %>&fmpf=<%=fmpf %>&fmrv=<%=fmrv %>&mode=DRAFT&fmnm=<%=fmnm %>&objectID=<%=objectID%>&sysManager=<%=sysManager%>" method="post">
       ...
       <input type="hidden" id="sysManager" name="sysManager" value="<%=sysManager%>" />
    </form>
</body>
</html>
