    ...
    if (document.location.href.indexOf('read.htm') > -1 && getInfo("commentlist").indexOf("rejectedtodept") > -1) {
        ...
        for (var i = 0; i < m_CommentList.length; i++) {
            ...
            if(m_CommentList[i].getAttribute("COMMENT").indexOf("rejectedtodept") > -1) {
                var insertDate = m_CommentList[i].getAttribute("INSERT_DATE").replace("T", " ").substring(0, m_CommentList[i].getAttribute("INSERT_DATE").replace("T", " ").indexOf("."));
                ...
                //2014-04-29 hyh 추가
                comment.setAttribute("comment_Dept", m_CommentList[i].getAttribute("COMMENT").split("&")[2]);
                //2014-04-29 hyh 추가 끝
...
function DownloadCOVIUpload() {
    var winstyle = "height=360, width=290, status=no, resizable=yes, help=no, scroll=yes";
...
function G_displaySpnAttInfo2(bRead) {//수정본
    ...
    if (!("ActiveXObject" in window)) document.getElementById("AttFileInfo").parentNode.children[0].innerHTML = m_oFormMenu.gLabel_AttachList;
...
var gz_Editor = "0"; //시스템 사용 에디터 0.TextArea, 1.DHtml, 2.TagFree, 3.XFree, 4.TagFree/XFree, 5.Activesquare, 6.CrossEditor, 7.ActivesquareDefault/CrossEditor -, 8.dext5
function UpdateImageData() {
    ...
    imgList = "";
    if (getInfo("editortype") == "3" || (getInfo("editortype") == "4" && !("ActiveXObject" in window))) {
    ...
    } else if (getInfo("editortype") == "2" || (getInfo("editortype") == "4" && ("ActiveXObject" in window))) {
    ...
    } else if (getInfo("editortype") == "6" || (getInfo("editortype") == "7" && !("ActiveXObject" in window))) {
        document.getElementById("dhtml_body").value = tbContentElement.GetHtmlValue();
        n = document.getElementById("dhtml_body").value.indexOf('http://') + 1;
    } else if (getInfo("editortype") == "5" || (getInfo("editortype") == "7" && ("ActiveXObject" in window))) {
        n = document.tbContentElement.GetFileNum(0);
    }else if (getInfo("editortype") == "8") {  //dext5 add
        document.getElementById("dhtml_body").value = tbContentElement.GetHtmlValue();
        n = document.getElementById("dhtml_body").value.indexOf('http://');
    }
...
function displayApvList(oApvList) {//debugger; //1212
    ...
    if (elmRoot != null) {
        ...
        if (m_oFormMenu.gProcessKind == "Cooperate" || m_oFormMenu.gProcessKind == true) {  //협조프로세스 20160317 jkh 추가 m_oFormMenu.gProcessKind == true
        ...
        if (elmListCount != 0) {
            ...
            */
            ...
            //2014-09-15 hyh 수정 끝  2016jkh 추가 수정 끝
...
function getCommentView(elmComment) {
    ...
    */
    ...
    if (document.getElementById("tdCommentView") != null) {
        ...
        document.getElementById("tdCommentView").innerHTML = "";
        //sCommentHtml = ""; //20160307 jkh수정 주석처리 
        //if (sCommentHtml!="") bCommentViewFirst=false;    // 2008.9.25 by 강효정 이 코드가 들어가면 인쇄버튼 누를 때마다 똑같은 의견이 중복되서 보이게 됨.
        if (bCommentViewFirst) {//alert(0);
            //sCommentHtml += '<table class="table_3" summary="결재의견" cellpadding="0" cellspacing="0">';
        ...
        $(elmComment).each(function (k, elm) {
            ...
            if ($(elm)[0].getAttribute("comment_Dept") == null) {
                ...
                */
                ...
                //20160307 수정 jkh시작
                    ...
                    if (getInfo("pdefname") == "ApprovalSystem-Request[0.0]") {
                        flag = true;
                    }
                    else {
                        flag = false;
            ...
            if (flag) {
                ...
                //2014-02-11 hyh 추가
                if ($(elm).parent().parent().parent().parent().attr("routetype") == "assist") {
                ...
                if (bProceed) {
                    ...
                    */
                    ...
                    if ((getInfo("fmpf") == "WF_FORM_ISU_ALL_COM010") || (getInfo("pdefname") == "ApprovalSystem-Draft[0.0]")) {
                        ...
                        if (document.location.href.indexOf('read.htm') > -1 && getInfo("commentlist").indexOf("rejectedtodept") > -1) {
                            ...
                            var m_CommentList = m_Comment.selectNodes("WF_COMMENT/comment_list");
                            for (var i = 0; i < m_CommentList.length; i++) {
                                var comment_list = m_CommentList[i].getAttribute("INSERT_DATE").replace("T", " ").substring(0, m_CommentList[i].getAttribute("INSERT_DATE").replace("T", " ").indexOf("."));
                                var commentList = m_CommentList[i].getAttribute("COMMENT");
                                if (m_CommentList[i].getAttribute("COMMENT").indexOf("rejectedtodept") > -1) {
                                    ...
                                    var dppathid = getInfo("dppathid").split("\\")[1];
                                    var dppathidcount = dppathid.length;
                                    var commentListUserID = m_CommentList[i].getAttribute("USER_ID");

                                    //sCommentHtml += '[' + formatDate(comment_list, "D") + ']&nbsp;&nbsp;';  //날짜   //+ sResult1 + '(' + sResult2 + ')  //결과상태값
                                    if ($(elm).attr("relatedresult") == "rejected" || $(elm).attr("relatedresult") == "rejectedto") {
                                        if ((getInfo("dpdsn") == dpdn.split(';')[0]) && (dppathid == commentListUserID.substring(0, dppathidcount))) {
                                            if (z > 0) {
                                                sCommentHtml = '';
                                                sCommentHtml += '<table class="table_8" summary="결재의견" cellpadding="0" cellspacing="0">';
                                                sCommentHtml += '<tr>';
                                                sCommentHtml += '<td align="left" style="font-size: 9pt;font-family: Arial, Dotum; padding-left: 10px;padding-top: 3px;padding-bottom: 3px;padding-right: 10px;"> ';
                                                sCommentHtml += '[' + formatDate(comment_list, "D") + ']&nbsp;&nbsp;';  //날짜   //+ sResult1 + '(' + sResult2 + ')  //결과상태값
                                                sStr = "의 반송사유";
                        ...
                        if ((getInfo("mode") == "COMPLETE") && (document.location.href.indexOf('read.htm') > -1 && getInfo("commentlist").indexOf("rejectedtodept") > -1)) {

                            sCommentHtml += '[' + formatDate($(elm).parent().attr("datecompleted"), "D") + ']&nbsp;&nbsp;';  //날짜   //+ sResult1 + '(' + sResult2 + ')  //결과상태값  //날짜   //+ sResult1 + '(' + sResult2 + ')  //결과상태값
                            sCommentHtml += m_oFormMenu.getLngLabel($(elm).parent().parent().attr("ouname"), false) + " " + m_oFormMenu.getLngLabel($(elm).parent().parent().attr("name"), false) + " " + sJobPosition + sStr;
                        ...
                        //20130911 hyh 수정 끝
                            ...
                            sCommentHtml += assistcmt.replace(/\\/g, "&#8361;"); //20161206
                            sCommentHtml += '</td>';
...
function setCommonLb() {
    ...
    var lb_Title = "";
    var lb_DocClassification = "";
    var lb_ConservationY = "";
    var lb_Attachment = "";
    var lb_RelatedDoc = "";
    var lb_Reference = "";
    var lb_DocExplanation = "";
    var lb_ReceiveDepart = "";
    var lb_OrdinaryDoc = "";
    var lb_DocNum = "";
    var lb_SecurityLvl = "";
    ...
    if (getInfo("uslng") == "ko-KR") {
        lb_Title = "제     목";//제&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;목
        lb_DocClassification = "문서분류";
        lb_ConservationY = "보존년한";
        lb_Attachment = "첨부파일";
        lb_RelatedDoc = "연결문서";
        lb_Reference = "참조";
        lb_DocExplanation = "문서설명";
        lb_ReceiveDepart = "수신부서";
        lb_OrdinaryDoc = "원 문 서";
        lb_DocNum = "문서번호";
        lb_SecurityLvl = "보안등급";
    }
    else if (getInfo("uslng") == "en-US") {
        lb_Title = "Title";
        lb_DocClassification = "Classification";
        lb_ConservationY = "Conservation";
        lb_Attachment = "Attachment";
        lb_RelatedDoc = "RelatedDoc";
        lb_Reference = "Reference";
        lb_DocExplanation = "Description Of Doc";
        lb_ReceiveDepart = "Department Received";
        lb_OrdinaryDoc = "Original Document";
        lb_DocNum = "Doc Number";
        lb_SecurityLvl = "Security Level";
    }
    else if (getInfo("uslng") == "ja-JP") {
        lb_Title = "タイトル";
        lb_DocClassification = "文書分類";
        lb_ConservationY = "保存期間";
        lb_Attachment = "添付ファイル";
        lb_RelatedDoc = "接続文書";
        lb_Reference = "参照";
        lb_DocExplanation = "文書の説明";
        lb_ReceiveDepart = "受信部";
        lb_OrdinaryDoc = "元の文書";
        lb_DocNum = "文書番号";
        lb_SecurityLvl = "セキュリティ評価";
    }
    else {//zh-CN
        lb_Title = "题     目";
        lb_DocClassification = "文件分类";
        lb_ConservationY = "保存年限";
        lb_Attachment = "附件";
        lb_RelatedDoc = "关联文件";
        lb_Reference = "参照";
        lb_DocExplanation = "文件说明";
        lb_ReceiveDepart = "收信部门";
        lb_OrdinaryDoc = "原始凭证";
        lb_DocNum = "文章编号";
        lb_SecurityLvl = "安全级别";
    }

	setvalue('lb_Title' , lb_Title);
//    document.getElementsByName("lb_Title")[0].value = lb_Title;    
	setvalue('lb_DocClassification' , lb_DocClassification);
//    document.getElementsByName("lb_DocClassification")[0].value = lb_DocClassification;
	setvalue('lb_ConservationY' , lb_ConservationY);
//    document.getElementsByName("lb_ConservationY")[0].value = lb_ConservationY;
	setvalue('lb_Attachment' , lb_Attachment);
//    document.getElementsByName("lb_Attachment")[0].value = lb_Attachment;
	setvalue('lb_RelatedDoc' , lb_RelatedDoc);
//    document.getElementsByName("lb_RelatedDoc")[0].value = lb_RelatedDoc;
	setvalue('lb_Reference' , lb_Reference);
//    document.getElementsByName("lb_Reference")[0].value = lb_Reference;
	setvalue('lb_DocExplanation' , lb_DocExplanation);
//    document.getElementsByName("lb_DocExplanation")[0].value = lb_DocExplanation;
	setvalue('lb_ReceiveDepart' , lb_ReceiveDepart);
//    document.getElementsByName("lb_ReceiveDepart")[0].value = lb_ReceiveDepart;
	setvalue('lb_OrdinaryDoc' , lb_OrdinaryDoc);
//    document.getElementsByName("lb_OrdinaryDoc")[0].value = lb_OrdinaryDoc;
	setvalue('lb_DocNum' , lb_DocNum);
//    document.getElementsByName("lb_DocNum")[0].value = lb_DocNum;
	setvalue('lb_SecurityLvl' , lb_SecurityLvl);
//    document.getElementsByName("lb_SecurityLvl")[0].value = lb_SecurityLvl;
    ...
    //if (D5edit_load == true) {
       //alert("true1");
        try {
            ...
            var d5Dom = DEXT5.getD5Dom();
            d5Dom.document.getElementById("tdSubject").innerHTML = "<p align='left'>&nbsp;" + subJecReplace(document.getElementsByName("SUBJECT")[0].value) + "</p>";


        } catch (e) { }
    //} else if (D5edit_load == false) {
    //    alert("false1");
    //    return;
    //}
}
function subJecReplace(value) {
    return value.replace(/\\/g, "￦").replace("&", "&amp;").replace("\'", "&apos;").replace("<", "&lt;").replace(">", "&gt;");
}
...
function SetDocNoTbxSync() {
   // alert("true2");
    ...
    try {
        ...
        var d5Dom = DEXT5.getD5Dom();
        d5Dom.document.getElementById("tdDocNum").innerHTML = "< p align = 'left' >& nbsp; " + document.getElementsByName("DocNumber")[0].value + "</p >";
        //var style = document.createElement("style");
        //style.appendChild(document.createTextNode("table { table-layout : fix;}"));
        //d5Dom.document.appendChild(style);
        } catch (e) { }
    //} else if (D5edit_load == false) {
    //    alert("false2");
    //    return;
...
function SetSaveTermTbxSync() {
    ...
    var repeat = setInterval(function () {
        if (D5edit_load) {
            SetSaveTermTbxSyncAfter();
            clearInterval(repeat);
        }
    }, 1000);
}
function SetSaveTermTbxSyncAfter() {
       try {
        ...
        if (vSaveTermChangeCnt == 0) {
            ...
            //var oIFrm = $("#dext5_design_dext5editor1");
        ...
        //dext5
        var d5Dom = DEXT5.getD5Dom();
        d5Dom.document.getElementById("tdSaveTerm").innerHTML = "<center>" + $("select[name=SAVE_TERM] option:selected").text()  + "</center>";
