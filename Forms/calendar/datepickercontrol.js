...
DatePickerControl.buttonTitle     = "OK";
...
if(window.location.href.indexOf("list") > -1 || window.location.href.indexOf("List") > -1){
    DatePickerControl.Months = gLabel_Months;
    DatePickerControl.Days = gLabel_Days;
}else if(window.location.href.indexOf("Popup") > -1){
    DatePickerControl.Months = opener.parent.menu.gLabel_Months;
    DatePickerControl.Days = opener.parent.menu.gLabel_Days;
}else{
    DatePickerControl.Months = parent.menu.gLabel_Months;
    DatePickerControl.Days = parent.menu.gLabel_Days;
}
...
{
    var newid;
    if(input.attributes.getNamedItem("name") != null){
	    newid = this.buttonIdPrefix + input.attributes.getNamedItem("name").value;
    }else{
	    newid = this.buttonIdPrefix + input.id;
    }
	...
	*/
	...
	// with this button and its format:
    if(input.attributes.getNamedItem("name") != null){
    	calButton.setAttribute("datepicker_inputid", input.attributes.getNamedItem("name").value);
    }else{
    	calButton.setAttribute("datepicker_inputid", input.id);//input.getAttribute("name").value
    }	
	calButton.setAttribute("datepicker_format", fmt);
	// Add the event listeners:
	if (calButton.addEventListener){
	...
	//calButton.style.height = input.offsetHeight+5;
	calButton.src = "/GWImages/Common/icon/icon_calendar.gif";
	if (this.buttonPosition == "in"){
...
DatePickerControl.show = function()
{
	if (!this.displayed){
		...
		this.calContainer.style.top        = top + Number(input.offsetHeight) + this.offsetY + "px";
		this.calContainer.style.left       = left + Number(input.offsetWidth) + this.offsetX + "px";
		this.calContainer.style.display    = "none";
...
DatePickerControl.writeDate = function(day)
{
	...
	//시작일, 끝날짜 기간계산
    if (this.inputControl.attributes.getNamedItem("name").value == "EDATE"){
		var strDate = f_Date(document.getElementsByName("SDATE")[0].value);
		var sDate = new Date(strDate);
		var sDay = sDate.getTime();
		
		strDate = f_Date(document.getElementsByName("EDATE")[0].value);
		var eDate = new Date(strDate);
		var eDay = eDate.getTime();
    
		if (sDay > eDay) {
			return false;
		}
		else {
			var sum = Math.round((eDay - sDay) / (1000*60*60*24)) + 1;
			document.getElementsByName("DAYS")[0].value = sum;
			document.getElementsByName("NIGHTS")[0].value=sum-1;
		}    
    }
...
DatePickerControl.onButtonClick = function(event)
{
	if (!this.displayed){
		...
		if(input == null)  input = document.getElementsByName(button.getAttribute("datepicker_inputid"))[0];
...
function f_Date(obj)
{
	var strDate, strObj, strYear, nMonth, strDay, nJinsu, nInitLen;
	strObj = obj;
	nJinsu = 10;
	nInitLen = 0;
...
//
// Compartir es la ?ica manera de perdurar
// EOF