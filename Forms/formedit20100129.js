...
function G_displaySpnAttInfo(bRead){//수정본
	...
	if (document.all['ATTACH_FILE_INFO'].value != ""){		
			...
			for (var i=0; i<elmList.length;i++) {
				...
				if (elm.getAttribute("location").indexOf(".") > -1 ){
					if (bReadOnly){
					    ...
					    }else{szAttFileInfo +=  displayFileName;}
					}else{
		...
		{
		    if(bReadOnly == "display"){
		            AttFileInfo.parentNode.children[0].innerHTML = "첨부파일";
		    }else{
...
function getRequestApvList(elmList,elmVisible,sMode,bReceive,sApvTitle,bDisplayCharge){//신청서html
	...
	for (var i=0; i<elmList.length-j;i++) {
        ...
        if(bReceive){ //담당부서 결재선
	        if(sApvTitle!=""){ 
	            ApvPOSLines += "<td rowspan='5' width='25' height='100' align='center' style='color: #662800; font-family: 돋움, Dotum;font-size: 12px; font-weight: bold; background: #f5ebe1;' nowrap='f'>"+(sApvTitle==undefined?parent.menu.gLabel_management+"<br>"+parent.menu.gLabel_dept:sApvTitle)+"</td>";}	            
        }else{
	        if(sApvTitle!=""){ 
	            ApvPOSLines += "<td rowspan='5' width='25' height='100' align='center' style='color: #662800; font-family: 돋움, Dotum;font-size: 12px; font-weight: bold; background: #f5ebe1;' nowrap='f'>"+(sApvTitle==undefined?parent.menu.gLabel_request+"<br>"+parent.menu.gLabel_dept:sApvTitle)+"</td>";}	            		
        }	        
	  }
		...
		if((bDisplayCharge && i==0 ) || ( elmTaskInfo.getAttribute("visible") != "n")) //결재선 숨기기한 사람 숨기기
		{
			if(elmTaskInfo.getAttribute("kind") == 'charge'){
				...
				ApvPOSLines += "<td height='20'  width='"+strwidth+"' align='center' style='color: #662800; font-family: 돋움, Dotum;font-size: 12px; font-weight: bold; background: #f5ebe1;'>" + temp_charge + "</td>";
			}
			else{	
				...
				try{
					...
					if(sTitle.substring(sTitle.lastIndexOf(";")+1) == "") sTitle = elm.getAttribute("position");	// kckangy update 2005.02.26
				...
				ApvPOSLines += "<td height='20'  width='"+strwidth+"' align='center' style='color: #662800; font-family: 돋움, Dotum;font-size: 12px; font-weight: bold; background: #f5ebe1;'>" + sTitle +"</td>";
			}		
			...
			switch( elmTaskInfo.getAttribute("kind")){
				...
				case "substitute":
						...
						ApvApproveNameLines += getPresence(sCode, i+sCode, elmname.getAttribute("sipaddress"))+ elmname.getAttribute("name")+interpretResult(elmTaskInfo.getAttribute("result"));	
				...
				case "bypass":	
						...
						ApvApproveNameLines += (LastApvName=="")? getPresence(sCode, i+sCode, elmname.getAttribute("sipaddress"))+elmname.getAttribute("name"):LastApvName;	
				...
				case "review": 	
						...
						ApvApproveNameLines += (strDate=="")?"&nbsp;":  getPresence(sCode, i+sCode, elmname.getAttribute("sipaddress"))+elmname.getAttribute("name")+interpretResult(elmTaskInfo.getAttribute("result"));
						ApvDateLines += (strDate=="")?"&nbsp;": formatDate(strDate);
						break;
				case "charge":						
						ApvSignLines += (strDate=="")?elm.getAttribute("name"): getSignUrl(sCode,elmTaskInfo.getAttribute("customattribute1"),elm.getAttribute("name"),strDate,false,elmTaskInfo.getAttribute("result"), true);
						ApvApproveNameLines += (strDate=="")?"&nbsp;":  getPresence(sCode, i+sCode, elmname.getAttribute("sipaddress"))+elmname.getAttribute("name")+interpretResult(elmTaskInfo.getAttribute("result"));
						ApvDateLines += (strDate=="")?"&nbsp;": formatDate(strDate);
						break;
				default :
	...
	Apvlines = "<table bgcolor='#FFFFFF' border='1' bordercolor='#dc5f0a' style='border-collapse: collapse;MARGIN-TOP: 0px;height:100%;'>" + Apvlines + "</table>";	//width:95%;
	return Apvlines;	
}
...
//			 alert(getInfo("loct"));
	        ...
	        if(getInfo("scPRec") =="1" || getInfo("scDRec") =="1" || getInfo("scChgr") == "1" || getInfo("scChgrOU") == "1"){//수신처가 있는경우 좌측:내부결재 우측 수신처 결재선
	            if(elmdiv.length > 1){
    			    ...
    			    if(i == 1) {
    			        ...
    			        try{RApvLine.innerHTML = getRequestApvList(elmList,elmVisible,"",true,sOUName,true);}catch(e){AppLine.innerHTML= getRequestApvList(elmList,"",true,elm.getAttribute("ouname"));}
    			    }
	            }else{
	                LApvLine.width = 600;
	                RApvLine.width = 200;
    			    if(i == 0) {
    			        try{LApvLine.innerHTML = getRequestApvList(elmList,elmVisible,"",false,null);}catch(e){AppLine.innerHTML= getRequestApvList(elmList,"",true,elm.getAttribute("ouname"));}
    			        //담당부서/담당업무뿌려주기    			        
					    RApvLine.innerHTML = "<table bgcolor='#FFFFFF' border='1' bordercolor='#dc5f0a' style='border-collapse: collapse;MARGIN-TOP: 0px;height:100;width:'><tr><td rowspan='5' width='25' height='100' align='center' style='color: #662800; font-family: 돋움, Dotum; font-size: 12px; font-weight: bold; background: #f5ebe1;' nowrap='f'>"+parent.menu.gLabel_managedept+"</td><td height='20' width='100' align='center' style='color: #662800; font-family: 돋움, Dotum; font-size: 12px; font-weight: bold; background: #f5ebe1;'>&nbsp;</td></tr><tr><td height='50' align='center' valign='middle' style='font-size:11pt;font-family: Georgia, Times New Roman, Times, serif,gulim;font-style:italic;color:#4584C9;font-weight:bold'>"+(getInfo("scChgr")=="1"?getInfo("scChgrV").split("@")[1]:"")+"</td></tr><tr><td height='20'  align='center' valign='middle' style='font-size:8pt;'></td></tr><tr><td height='20' align='center' valign='middle' style='font-size:8pt;'></td></tr></table>";
    			    }
	            }
	        }else{
	            if(elmdiv.length > 1){
		...
		if(elmListCount!=0)
		{
			Apvlines = "<tr><td height='20' width='20%' align='center' style='color: #662800; font-family: 돋움, Dotum;font-size: 12px; font-weight: bold; background: #f5ebe1;'>"+parent.menu.gLabel_dept+"</td>";
			Apvlines += "	<td height='20' width='20%' align='center' style='color: #662800; font-family: 돋움, Dotum;font-size: 12px; font-weight: bold; background: #f5ebe1;'>"+parent.menu.gLabel_username+"</td>";
			Apvlines += "	<td height='20' width='15%' align='center' style='color: #662800; font-family: 돋움, Dotum;font-size: 12px; font-weight: bold; background: #f5ebe1;'>"+parent.menu.gLabel_approvdate+"</td>";
			Apvlines += "   <td height='20' width='45%' align='center' style='color: #662800; font-family: 돋움, Dotum;font-size: 12px; font-weight: bold; background: #f5ebe1;'>"+parent.menu.gLabel_comment+"</td></tr>";
			for (var i=0; i<elmList.length;i++) {
				elm = elmList.nextNode();
				var sCode="";
				var sTitle="";
				...
				switch( elmTaskInfo.getAttribute("kind"))
				{
					case "substitute":							
							...
							break;
					case "bypass":							
							Apvlines +="<tr ><td height='20' style='font-size:9pt; padding-left:4;'>" + elm.getAttribute("ouname") + "</td>";//이준희(2007-07-06): 문서창 상단에서 협조자의 의견이 잘리지 않도록 수정함.
							Apvlines += "<td style='font-size:9pt; padding-left:4;'>";
							Apvlines += sTitle + " "+parent.menu.gLabel_substitue+" " + LastTitle + "</td>";
							Apvlines +="<td align='center' style='font-size:9pt;'>"+LastResult+"</td>";
					...
					default:
							Apvlines +="<tr ><td height='20' style='font-size:9pt; padding-left:4;'>" + elm.getAttribute("ouname") + "</td>";//이준희(2007-07-06): 문서창 상단에서 협조자의 의견이 잘리지 않도록 수정함.
							Apvlines +="<td style='font-size:9pt; padding-left:4;'>" + sTitle + "</td>";//이준희(2007-07-06): 문서창 상단에서 협조자의 의견이 잘리지 않도록 수정함.
							Apvlines +="<td align='center' style='font-size:9pt;'>"+((strDate=="")?"&nbsp;":formatDate(strDate) + interpretResult(elmTaskInfo.getAttribute("result")))+"</td>";
							Apvlines +="<td style='font-size:9pt;padding-left:4; word-break:break-all'>";	
			...
			for (var i=0; i<elmOUList.length;i++) {					
				...
				Apvlines +="<tr><td height='20' style='font-size:9pt;padding-left:4;'>"+elm.getAttribute("name")+"</td><td valign='middle' style='font-size:9pt;padding-left:4;'>";
				elmTaskInfo = elm.selectSingleNode("taskinfo");
				strDate = elmTaskInfo.getAttribute("datecompleted");
				if(strDate == null) {strDate = "";}
				//Apvlines += (strDate=="")?"&nbsp;</td>":interpretResult(elmTaskInfo.getAttribute("result"))+"</td>";
				Apvlines += elm.getAttribute("name")+"</td>";
		...
		for(var i=0;i<ccInfos.length;i++){
			...
			while(cc!=null){
				...
				if(cc.nodeName == "person"){
					sList+=(sList.length>0?";":"")+cc.getAttribute("ouname")+" "+getSplitted(cc.getAttribute("level"),";",1)+" "+cc.getAttribute("name");
        ...
        for(var i=0; i<elmList.length; i++){
            ...
            if(getInfo("scPRec") == 0 && getInfo("scDRec") == 0 && getInfo("scIPub") == 0)//일반결재
            {
                Apvlines +=   "<table><tr>" 
				    ...
				    +"	<tr >"
				    +"		<td  height='20'  width='4%'  style='color: #662800; font-family: 돋움, Dotum;font-size: 12px; font-weight: bold; background: #f5ebe1;'>" + parent.menu.gLabel_no + "</td>" //순  번 
				    +"		<td  height='20'  width='15%' style='color: #662800; font-family: 돋움, Dotum;font-size: 12px; font-weight: bold; background: #f5ebe1;'>" + parent.menu.gLabel_dept + "</td>" //부서3
				    +"		<td  height='20'  width='21%' style='color: #662800; font-family: 돋움, Dotum;font-size: 12px; font-weight: bold; background: #f5ebe1;' >" + parent.menu.gLabel_username + "</td>" //성  명4
				    +"		<td  height='20'  width='10%' style='color: #662800; font-family: 돋움, Dotum;font-size: 12px; font-weight: bold; background: #f5ebe1;' >" + parent.menu.gLabel_jobtitle +"</td>" //직  책5
				    +"		<td  height='20'  width='10%' style='color: #662800; font-family: 돋움, Dotum;font-size: 12px; font-weight: bold; background: #f5ebe1;' >" + parent.menu.gLabel_kind +"</td>" //종  류
				    +"		<td  height='20'  width='10%' style='color: #662800; font-family: 돋움, Dotum;font-size: 12px; font-weight: bold; background: #f5ebe1;' >" + parent.menu.gLabel_state + "</td>" //상태2
				    +"		<td  height='20'  width='9%' style='color: #662800; font-family: 돋움, Dotum;font-size: 12px; font-weight: bold; background: #f5ebe1;' >" + parent.menu.gLabel_approvdate + "</td>" // 결재일자
				    +"		<td  height='20'  width='21%' style='color: #662800; font-family: 돋움, Dotum;font-size: 12px; font-weight: bold; background: #f5ebe1;' >" + parent.menu.gLabel_oriapprover + "</td>" //	원결재자  				    
				    +"	</tr>"
				    + rtnPOSLine + "</table><br>";
...
//							}
		        ...
		        if(DeptState == "T") //내부결재 진행중에만 보이기
		        {
	                if(ApvCmt == "" || ApvCmt == "&nbsp;")
	                {
	                    ApvPOSLines = "<tr id='INDEPT" + parentCnt + "' style='font-family: 돋움, Dotum;font-size: 12px;color: #142460; text-align: left; letter-spacing: -0.3pt; text-decoration: none;'><td style='background-color:#f5ebe1; color:#142460;'>" + parentCnt + "-" + cnt + "</td><td style='background:#FFFFFF;font-size:9pt;' height='20'>" + ApvDept + "</td>" + "<td style='background:#FFFFFF;font-size:9pt;' >" + ApvSignLines +  "</td><td style='background:#FFFFFF;font-size:9pt;' >" + sTitle 
	                                   +  "<td style='background:#FFFFFF;font-size:9pt;'>" + ApvKind + "</td><td style='background:#FFFFFF;font-size:9pt;' height='20'>" + ApvState +  "</td>" +  "</td><td style='background:#FFFFFF;font-size:9pt;' >" + ApvDateLines +  "</td><td style='background:#FFFFFF;font-size:9pt;'>" + OriginApprover +  "</td></tr>" + ApvPOSLines;
	                    cnt++; 
	                }
	                else
	                ...
	                if(ApvCmt == "" || ApvCmt == "&nbsp;")
	                {
	                    ApvPOSLines = "<tr id='INDEPT" + parentCnt + "' style='font-family: 돋움, Dotum;font-size: 12px;color: #142460; text-align: left; letter-spacing: -0.3pt; text-decoration: none;display:none;'><td style='background-color:#f5ebe1; color:#142460;'>" + parentCnt + "-" + cnt + "</td><td style='background:#FFFFFF;font-size:9pt;' height='20'>" + ApvDept + "</td>" + "<td style='background:#FFFFFF;font-size:9pt;' >" + ApvSignLines +  "</td><td style='background:#FFFFFF;font-size:9pt;' >" + sTitle 
	                                   +  "<td style='background:#FFFFFF;font-size:9pt;'>" + ApvKind + "</td><td style='background:#FFFFFF;font-size:9pt;' height='20'>" + ApvState +  "</td>" +  "</td><td style='background:#FFFFFF;font-size:9pt;' >" + ApvDateLines +  "</td><td style='background:#FFFFFF;font-size:9pt;'>" + OriginApprover +  "</td></tr>" + ApvPOSLines;
	                    cnt++; 
	                }
	                else
...
function getCommentView(elmComment)
{
        ...
        if (bCommentViewFirst){
            sCommentHtml+='<table  border="1" bordercolor="#dc5f0a" id="com_tbl" cellpadding="0" cellspacing="0" style="border-collapse: collapse;" align="center" width="100%">';
            sCommentHtml +='<tr><td width="18%" height="20" align="center" style="color: #662800; font-family: 돋움, Dotum;font-size: 12px; font-weight: bold; background: #f5ebe1;">결재구분</td>';
            sCommentHtml +='<td width="21%" height="20" align="center" style="color: #662800; font-family: 돋움, Dotum;font-size: 12px; font-weight: bold; background: #f5ebe1;">결재자</td>';
            sCommentHtml +='<td width="16%" height="20" align="center" style="color: #662800; font-family: 돋움, Dotum;font-size: 12px; font-weight: bold; background: #f5ebe1;">결재일시</td>';
            sCommentHtml +='<td width="45%" height="20" align="center" style="color: #662800; font-family: 돋움, Dotum;font-size: 12px; font-weight: bold; background: #f5ebe1;">결재의견</td></tr>';
        }
        var elm,sGubun;
        ...
        {  
            ...
            sCommentHtml += '<td style="font-size: 9pt;font-family: "돋움", Dotum; padding-left: 10px;">'+ sResult1 +'('+sResult2+')</td>';
            sCommentHtml += '<td style="font-size: 9pt;font-family: "돋움", Dotum; padding-left: 3px;">'+elm.parentNode.parentNode.getAttribute("ouname") +'/'+elm.parentNode.parentNode.getAttribute("name") +'/'+ sViewJob+'</td>';
            //sCommentHtml += '<td style="font-size: 9pt;padding-left: 3px;">'+ formatDate2(elm.getAttribute("datecommented")) +'</td>';
            sCommentHtml += '<td style="font-size: 9pt;font-family: "돋움", Dotum; padding-left: 3px;">'+ formatDate2(elm.parentNode.getAttribute("datecompleted")) +'</td>';
            sCommentHtml += '<td style="font-size: 9pt;font-family: "돋움", Dotum; padding-left: 3px;">'+ assistcmt +'</td>';               
            sCommentHtml +='</tr>';
        }
        tdCommentView.style.display = "";
