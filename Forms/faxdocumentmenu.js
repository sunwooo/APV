...
function disableBtns(bDisable){
	...
	document.getElementById('btPreView').disabled = bDisable;
}
...
function getFormInfosXML(){
	var forminfos = "";
	var ssubject = m_oFormEditor.document.getElementsByName('SUBJECT')[0].value.replace(/'/g, "&apos;");

	forminfos = "<ClientAppInfo><App name='FormInfo'><forminfos><forminfo "
	+" prefix='" + getInfo("fmpf") + "'"
	+" revision='" +  getInfo("fmrv") + "'"
	+" instanceid='" +  getInfo("fiid") + "'"
	+" id='" +  getInfo("fmid") + "'"
	+" name='" +  getInfo("fmnm") + "'"
	+" schemaid='" +  getInfo("scid") + "'"
	+" index='0'"
	+" filename='" +  getInfo("fmfn") + "'"
	+" subject='" +  ssubject + "'"	
	//+" urgentreason='" +  field["REASON"].value + "'"
	+ " req_response='" + ((m_oFormEditor.document.getElementsByName('REQ_RESPONSE')[0] == undefined) ? "" : m_oFormEditor.document.getElementsByName('REQ_RESPONSE')[0].value) + "'"
	+" /></forminfos></App></ClientAppInfo>";	
	return forminfos;
}
function getFormInfoXML(){ //양식정보추가
    var m_oFormInfos = CreateXmlDocument();
	m_oFormInfos.async = false;
	m_oFormInfos.loadXML("<?xml version='1.0' encoding='utf-8'?>"+getInfo("pidc"));
		...
		switch(obj.id){
		    case "btSave":		    
		        if (m_oFormEditor.document.getElementsByName("SUBJECT")[0].value == ""){
                    alert(gMessage28); 
                    m_oFormEditor.document.getElementsByName("SUBJECT")[0].focus();
                    return false;
		        } else if (m_oFormEditor.document.getElementsByName("ZIP_CODE")[0].value == ""){
		            alert("우편번호를 입력하세요.");
		            m_oFormEditor.document.getElementsByName("ZIP_CODE")[0].focus();
		            return false;		       
		        } else if (m_oFormEditor.document.getElementsByName("ADDRESS")[0].value == ""){
		            alert("주소를 입력하세요.");
		            m_oFormEditor.document.getElementsByName("ADDRESS")[0].focus();
		            return false;
		        } else if (m_oFormEditor.document.getElementsByName("PHONE")[0].value == ""){
		            alert("전화번호를 입력하세요.");
		            m_oFormEditor.document.getElementsByName("PHONE")[0].focus();
		            return false;
		        } else if (m_oFormEditor.document.getElementsByName("FAX")[0].value == ""){
		            alert("FAX를 입력하세요.");
		            m_oFormEditor.document.getElementsByName("FAX")[0].focus();
		            return false;
		        } else if (m_oFormEditor.document.getElementsByName("CHARGE")[0].value == ""){
		            alert("담당을 입력하세요.");
		            m_oFormEditor.document.getElementsByName("CHARGE")[0].focus();
		            return false;
		        } else if (m_oFormEditor.document.getElementsByName("DOC_NO")[0].value == ""){
		            alert("문서번호를 입력하세요.");
		            m_oFormEditor.document.getElementsByName("DOC_NO")[0].focus();
		            return false;
		        } else if (m_oFormEditor.document.getElementsByName("DATED")[0].value == ""){
		            alert("발행일자를 선택하세요.");		           
		            return false;
		        } else if (m_oFormEditor.document.getElementsByName("RECEIVE")[0].value == ""){
		            alert("수신을 입력하세요.");
		            m_oFormEditor.document.getElementsByName("RECEIVE")[0].focus();
		            return false;
		        } else if (m_oFormEditor.document.getElementsByName("REFERENCE")[0].value == ""){
		            alert("참조를 입력하세요.");
		            m_oFormEditor.document.getElementsByName("REFERENCE")[0].focus();
		            return false;
		        }
		        ...
		        if (getInfo("fmpf") == "OFFICIAL_DOCUMENT_03") { //수기문서
		            ...
		            sRecmt = makeNode("approval", m_oFormEditor.document.getElementsByName('APPROVE')[0].value)
				        ...
				        + makeNode("kind", "수기");
		        } else if (getInfo("fmpf") == "DRC_FORM_OFFICIAL_DOCUMENT") { //대외공문발송
		            sRecmt = "수기";
		        ...
		        var sItem = "<request>"		      
				 ...
				 + makeNode("rdpdn", m_oFormEditor.document.getElementsByName('RECEIVE')[0].value)
...
//		        requestProcess("SAVE");
            ...
            case "btPrintView":  //인쇄 미리보기           
                if (m_oFormEditor.location.pathname.indexOf('read.htm') > -1) {
                    printDiv = m_oFormEditor.bodytable.innerHTML; iWidth = 800; iHeight = 700; sUrl = "PrintForm_OutDoc.htm";
