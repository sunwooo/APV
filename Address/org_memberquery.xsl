		...
		<xsl:if test="(count(//ORG_PERSON)+count(//ORG_ADDITIONALJOB) +count(//ORG_DISPATCHEDJOB)+count(//ORG_UNIT)) > 0 ">
			<xsl:element name="addresslist">
				<xsl:for-each select="ORG_PERSON">
					...
					<xsl:element name="FLDP"><xsl:value-of select="UNIT_FULL_NAME" /></xsl:element>
					<xsl:element name="JD"><xsl:value-of select="UNIT_CODE" /></xsl:element>
					<xsl:element name="LN"></xsl:element>
					<xsl:element name="FN"><xsl:value-of select="DISPLAY_NAME" /></xsl:element>
					<xsl:element name="TL"><xsl:value-of select="substring-after(JOBTITLE_Z,'&amp;')" /></xsl:element>
					<xsl:element name="PO"><xsl:value-of select="substring-after(JOBPOSITION_Z,'&amp;')" /></xsl:element>
					<xsl:element name="LV"><xsl:value-of select="substring-after(JOBLEVEL_Z,'&amp;')" /></xsl:element>
					...
					<xsl:element name="UGNM"><xsl:value-of select="NAME" /></xsl:element>
					...
					<xsl:element name="ETNM"><xsl:value-of select="ENT_NAME" /></xsl:element>
					...
					<xsl:element name="SIP"><xsl:value-of select="SIPADDRESS" /></xsl:element>	
					</xsl:element>	
				</xsl:for-each>
				<xsl:for-each select="ORG_ADDITIONALJOB">
					...
					<xsl:element name="DP"><xsl:value-of select="NAME" /></xsl:element>
					<xsl:element name="OF"></xsl:element>
					...
					<xsl:element name="SG"><xsl:value-of select="UNIT_CODE" /></xsl:element>
					<xsl:element name="SGNM"><xsl:value-of select="NAME" /></xsl:element>
					<xsl:element name="UG"><xsl:value-of select="PARENT_UNIT_CODE" /></xsl:element>
					<xsl:element name="UGNM"><xsl:value-of select="NAME1" /></xsl:element>
					<xsl:element name="DEPUTY"><xsl:value-of select="DEPUTY" /></xsl:element>
					<xsl:element name="RV1"><xsl:value-of select="RESERVED1" /></xsl:element>
					<xsl:element name="RV2"><xsl:value-of select="RESERVED2" /></xsl:element>
					<xsl:element name="OT"><xsl:value-of select="OFFICE_TEL" /></xsl:element>
					...
					<xsl:element name="USEC"><xsl:value-of select="SABUN" /></xsl:element>
					<xsl:element name="USCC"><xsl:value-of select="COST_CENTER" /></xsl:element>
					<xsl:element name="JDT"><xsl:value-of select="JOIN_DATE" /></xsl:element>
					<xsl:element name="SIP"><xsl:value-of select="SIPADDRESS" /></xsl:element>
					</xsl:element>
				</xsl:for-each>
...
<xsl:attribute name="po"><xsl:value-of select="concat(concat(substring-before(JOBPOSITION_Z,'&amp;'),';'),substring-after(JOBPOSITION_Z,'&amp;'))" /></xsl:attribute>
	...
	<xsl:attribute-set name="itemSetAdd" >
		...
		<xsl:attribute name="lv"><xsl:value-of select="concat(concat(LEVEL_CODE,';'), LEVEL_NAME)" /></xsl:attribute>
	</xsl:attribute-set>
	<xsl:attribute-set name="itemSetDis" >
	<xsl:attribute name="so"><xsl:value-of select="B/C/D/SORT_KEY" /></xsl:attribute>
	<xsl:attribute name="tl"><xsl:value-of select="concat(concat(TITLE_CODE,';'), TITLE_NAME)" /></xsl:attribute>
	<xsl:attribute name="po"><xsl:value-of select="concat(concat(PSTN_CODE,';'), PSTN_NAME)" /></xsl:attribute>
	<xsl:attribute name="lv"><xsl:value-of select="B/C/D/JOBLEVEL_Z" /></xsl:attribute>
	</xsl:attribute-set>
	<xsl:attribute-set name="itemSetUnit" >
	<xsl:attribute name="so"><xsl:value-of select="SORT_KEY" /></xsl:attribute>
	<xsl:attribute name="tl"><xsl:value-of select="concat(concat('manager',';'), '')"/></xsl:attribute>
