    ...
    <xsl:element name="response">
      ...
      <xsl:for-each select="//Table1">
        ...
        <xsl:element name="forminstance">
          ...
          <xsl:element name="picreatorid"><xsl:value-of select="SENDER_ID"/></xsl:element>
          <xsl:element name="picreator"><xsl:value-of select="SENDER_NAME"/></xsl:element>
          ...
          <xsl:element name="picreatordept"><xsl:value-of select="SENDER_OU_NAME"/></xsl:element>
          <xsl:element name="picreatordeptid"><xsl:value-of select="SENDER_OU_ID"/></xsl:element>
          <xsl:element name="pibd1"><xsl:value-of select="BUSINESS_DATA1"/></xsl:element>

          <xsl:element name="TOT_CNT"><xsl:value-of select="TOT_CNT"/></xsl:element>
          <xsl:element name="READED_CNT"><xsl:value-of select="READED_CNT"/></xsl:element>
          <xsl:element name="UNREADED_CNT"><xsl:value-of select="UNREADED_CNT"/></xsl:element>
        </xsl:element>
        <!--</xsl:if>-->
      </xsl:for-each>
    </xsl:element>
