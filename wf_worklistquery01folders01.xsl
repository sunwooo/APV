		...
		<xsl:for-each select="//Table1">
			...
			<!--<xsl:if test=" ( position() > ( $iPage - 1 )*$iPageSize ) and ($iPage*$iPageSize  >= position() ) ">-->
				<xsl:element name="workitem">
					...
					<xsl:element name="folderid"><xsl:value-of select="FOLDER_ID"/></xsl:element>
					<xsl:element name="foldernm"><xsl:value-of select="FOLDER_NAME"/></xsl:element>
					...
					<xsl:element name="parentfolderid"><xsl:value-of select="PARENTS_ID"/></xsl:element>
				</xsl:element>
			<!--</xsl:if>-->
		</xsl:for-each>
