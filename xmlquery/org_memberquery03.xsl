...
<xsl:template match="ROOT">
			...
			<xsl:element name="addresslist">
				...
				<xsl:for-each select="ORG_ADDITIONALJOB">
					<xsl:element name="item" use-attribute-sets="itemSetAdd">
						...
						<xsl:element name="TL"><xsl:value-of select="B/C/D/E/@NAME" /></xsl:element>
						<xsl:element name="PO"><xsl:value-of select="B/C/D/E/F/@NAME" /></xsl:element>
						<xsl:element name="LV"></xsl:element>
						<xsl:element name="AN"><xsl:value-of select="@PERSON_CODE" /></xsl:element>
						<xsl:element name="PI"><xsl:value-of select="@PERSON_ID" /></xsl:element>
						<xsl:element name="CP"></xsl:element>
						<xsl:element name="DP"><xsl:value-of select="B/@NAME" /></xsl:element>
						<xsl:element name="OF"></xsl:element>
	...
	<xsl:attribute name="tl"><xsl:value-of select="concat(concat(@TITLE_CODE,';'), B/C/D/E/@NAME)" /></xsl:attribute>
	<xsl:attribute name="po"><xsl:value-of select="concat(concat(@PSTN_CODE,';'), B/C/D/E/F/@NAME)" /></xsl:attribute>
	<xsl:attribute name="lv"><xsl:value-of select="concat(concat(@LEVEL_CODE,';'),@LEVEL_CODE)" /></xsl:attribute>
	</xsl:attribute-set>
	<xsl:attribute-set name="itemSetUnit" >
	<xsl:attribute name="so"><xsl:value-of select="@SORT_KEY" /></xsl:attribute>
	<xsl:attribute name="tl"><xsl:value-of select="concat(concat('manager',';'), '팀장')" /></xsl:attribute>
