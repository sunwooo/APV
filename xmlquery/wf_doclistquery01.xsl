...
<msxsl:script language="JScript" implements-prefix="cfxsl">
 ...
 <xsl:template match="ROOT">
	<xsl:element name="response">
		<xsl:element name="totalcount"><xsl:value-of select="count(//WF_DOC_REG_LIST)"/></xsl:element>
		<xsl:element name="totalpage"><xsl:value-of select="cfxsl:getPageValue($iPageSize,count(//WF_DOC_REG_LIST))"/></xsl:element>
