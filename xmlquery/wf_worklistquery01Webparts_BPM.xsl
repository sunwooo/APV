		...
		<xsl:for-each select="WF_WORKITEM_LIST">
			<xsl:sort select="string(concat('@',$sortby))" order="descending"/>
				<xsl:element name="workitem">
					...
					<xsl:element name="pname"><xsl:value-of select="@PNAME"/></xsl:element>
					<xsl:element name="projname"><xsl:value-of select="@PROJNAME"/></xsl:element>
					<xsl:element name="due_date"><xsl:value-of select="@DUE_DATE"/></xsl:element>
					<xsl:element name="activity_name"><xsl:value-of select="@ACTIVITY_name"/></xsl:element>
					<xsl:element name="status"><xsl:value-of select="@STATUS"/></xsl:element>
					<xsl:element name="work_div"><xsl:value-of select="@WORK_DIV"/></xsl:element>
					<xsl:element name="work_cnt"><xsl:value-of select="@WORK_CNT"/></xsl:element>
				</xsl:element>
		</xsl:for-each>
	</xsl:element>
</xsl:template>
