...
var Receipt_id;var Receipt_name;var Receipt_ou_id;var Receipt_ou_name;var Receipt_state;var User_chk;
...
function doButtonAction(obj)
{
	...
	switch (obj.id)
	{
		case "btCirculation" ://회람자
		    insertToList(switchParentNode(3));  m_oUserList.document.getElementsByName("chkall")[0].checked = false; break;
		case "btCirculationDept" ://회람부서
		    bDept = true;	insertToList(switchParentNode(4));  break;
		case "btClose" :
		    window.close(); break;
		case "btSend":  //삭제처리,추가처리  
		    if (ideas.value == ""){alert(strMsg_064); return;}    
		    SaveDB();break;
		case "btDeleteccrec":
...
function insertToList(oList)
{
	...
	dataBind("new",xmldom, document.getElementById("tblrefinfo"));bDept = false;
}
...
function dataBind (mode,xmldom, oTable)
{
	...
	switch (mode)
	{
		...
		case "new":
		    ...
		    if(col.length > 0 && oTable.rows.length > 0 && oTable.rows[0].innerText == strMsg_136){//"항목이 없습니다."
		        resetGird(oTable);
		    }else if(col.length == 0 ){
                alert(strMsg_162);//"하나 이상의 대상자를 선택하세요.");	
		    }
			...
			for (var i=0;i<col.length ;i++)
			{
				...
				}else
				{
				    if(bFirst)
				    {
				        alert(strMsg_171);//"이미 배포수신자 이거나 결재선에 지정된 사람 입니다.\r\n배포 그룹에 추가 할 수 없습니다.");	
				        bFirst=false;
				    }
				}
			}
...
{   //chkAllref회람 chkAllccrec/chkDel수신/참조
    ...
    try{
        var ochkDel = document.getElementsByName(szchkDel);
        if ( ochkDel != null){
            var ochkAll = document.getElementById(szchkAll);
            if(ochkDel.length == null){
