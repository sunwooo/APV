    ...
    if (admintype == "MONITOR") {
        ...
        switch (gLocation) {
            ...
            case "COMPLETE":
                ...
                aXML += "<param><name>PI_NAME</name><type>varchar</type><length>1024</length><value><![CDATA[" + PI_NAME + "]]></value></param>";
                aXML += "<param><name>PI_INITIATOR_NAME</name><type>varchar</type><length>256</length><value><![CDATA[" + PI_INITIATOR_NAME + "]]></value></param>";
                aXML += "<param><name>PI_ETC</name><type>varchar</type><length>256</length><value><![CDATA[" + PI_ETC + "]]></value></param>";
...
//                    PF_PERFORMER_ID = strDeptID;
            ...
            switch (gLocation) {
                ...
                case "PREAPPROVAL": if (pXML == null) pXML = "exec dbo.usp_wf_worklistquery01preapproval "; //PREAPPROVAL
                case "APPROVAL": if (pXML == null) pXML = "exec dbo.usp_wf_worklistquery01approval "; //APPROVAL
                case "CONSULT": if (pXML == null) pXML = "exec dbo.usp_wf_worklistquery01consult ";
                case "PROCESS": if (pXML == null) pXML = "exec dbo.usp_wf_worklistquery01process "; //PROCESS
                case "TODO": if (pXML == null) pXML = "exec dbo.usp_wf_worklistquery01todo "; //TODO
                case "CANCEL": if (pXML == null) pXML = "exec dbo.usp_wf_worklistquery01cancel ";
                case "REJECT": if (pXML == null) pXML = "exec dbo.usp_wf_worklistquery01reject ";
                case "CCINFO": if (pXML == null) pXML = "exec dbo.usp_wf_worklistquery01ccinfo ";
                case "REVIEW1": if (pXML == null) pXML = "exec dbo.usp_wf_worklistquery01review1 ";
                ...
                case "JOBFUNCTION":
                    ...
                    if (gLocation == "JOBFUNCTION") {
                        ...
                        if (GUBUN != '' || GROUP_KIND != '' || TITLE != '' || ORDERFILED != '' || ORDERDESC != '' || d_QSDATE != '') {
                            pXML = "exec dbo.usp_wf_worklistquery01"; //(수정)
                        } else {
                    ...
                    case "DEPART":
                        ...
                        switch (strBox) {
                            ...
                            case "AD":
                                ...
                                break;
                            case "D":
                                strXSLFile = "Circulation/wfform_tonccquery01.xsl"; //회람수신함
                                break;
                            default:
    ...
    if (m_xmlHTTPXSL.readyState == 4) {
        ...
        if (m_xmlHTTPXSL.responseText.charAt(0) == '\r') {
            ...
            if (gLocation == "APPROVAL" || gLocation == "PROCESS" || (gLocation == "JOBFUNCTION" && (gLocationMode == "APPROVAL" || gLocationMode == "PROCESS"))) {
                ...
                if (g_totalcount > 0) {
                    for (var x = 2; x < g_eGalTable.rows.length; x++) {
            ...
            //체크박스 활성화
            if (uid != 'MONITOR' && (gLocation == "APPROVAL" || gLocation == "COMPLETE" || gLocation == "REJECT" || gLocation == "CCINFO" || gLocation == "TCINFO" || gLocation == "REVIEW3" || (gLocation == "JOBFUNCTION" && (gLocationMode == "APPROVAL")) || gLocation == "UFOLDER" || (gLocation == "DEPART" && (strBox == "A" || strBox == "E" || strBox == "S" || strBox == "RC" || strBox == "I" || strBox == "R")))) {
...
function event_GalTable_onmousedown(e) {
    ...
    if (el.tagName == "IMG" && (el.attributes["covimode"].nodeValue != null && el.attributes["covimode"].nodeValue == 'imgctxmenu')) {
    } else {
...
function processSelectedRow() {
    if (getRowAttribute(g_eCurrentRow, "fmid") != "") {
        //<유미
        /*
        if (gLocation=="TCINFO" && getRowAttribute(g_eCurrentRow,"kind")=="2")
        {
        settingTCINFO();            
        }
        */
        ...
        var strURL = "Forms/Form.aspx?mode=" + getRowAttribute(g_eCurrentRow, "mode")
					...
					+ "&admintype=" + admintype + "&archived=" + bArchived + "&pidc=" + "&pibd1="; // + toUTF8(getRowAttribute(g_eCurrentRow,"pibd1"));//toUTF8(getRowAttribute(g_eCurrentRow,"pidc")) + 2006.12.13 by wolf 사용자 문서 조회 및 수정 파라미터 추가		
...
function delete_onClick(bFolder) {
...
function groupby() {
    ...
    if (skind.value == "total") {
        szSrc = "listitemstodo.aspx?uid=" + uid + "&location=" + gLocation + "&label=" + strlabel; //수정
    }
...
function cmdapprove_OnClick(APVLIST) {
    if (document.getElementsByName("chkAPV") != null) {
        ...
        if (strflag == false) {
            ...
            if (bMulti) {
                ...
                requestHTTP("POST", sUrl, true, "text/xml", receiveApproveQuery, sItems);
            }
        }
    }
}
...
function foldermove_OnClick(gubun) {
    ...
    openWindow(strURL, "FOLDERFORMS", 300, 350, 'resize');
}
...
function makefolderinfo(item, szfolderid) {
    ...
    sItems += "<linkurl><![CDATA[" + item.getAttribute("pidc") + ";" + item.getAttribute("piid") + ";" + item.getAttribute("bstate") + "]]></linkurl>";
