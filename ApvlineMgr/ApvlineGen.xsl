...
<xsl:stylesheet version="1.0"
  ...
  xmlns:cfxsl="http://www.covision.co.kr/xslt/coviflow">
	...
	<xsl:param name="steptype">division</xsl:param>
	<xsl:param name="divisiontype">send</xsl:param>
	...
	<xsl:param name="ounametype"></xsl:param>
	<xsl:param name="splittype"></xsl:param>
  <xsl:param name="kindext"></xsl:param>
  <xsl:param name="stampRights">N</xsl:param> <!-- 인장권자여부 (2012-12-17 HIW) -->
	...
	<xsl:template match="/">
		<steps>
      <xsl:choose>
        <xsl:when test="$steptype='division' ">
          <xsl:call-template name="tpl_division">
  ...
  <xsl:template name="tpl_division" match="*">
    <xsl:param name="divisiontype" />
    <division>
      <xsl:attribute name="divisiontype"><xsl:value-of select="$divisiontype"/></xsl:attribute>
      <xsl:attribute name="name"><xsl:value-of select="$referencename"/></xsl:attribute>
      <xsl:choose>
        <xsl:when test="$routetype='receive'">
          <xsl:attribute name="oucode">
            <xsl:value-of select="AN"/>
          </xsl:attribute>
          <xsl:attribute name="ouname">
      ...
      <xsl:choose>
        <xsl:when test="$routetype='approve' and $allottype='parallel' ">
          <xsl:call-template name="tpl_step_joined">
            <xsl:with-param name="visible" select="$childvisible"/>
          </xsl:call-template>
        </xsl:when>
        <xsl:when test="$routetype='approve' and $allottype='' ">
          <xsl:call-template name="tpl_step_splitted"/>
        </xsl:when>
        <xsl:when test="$routetype='consult' and $splittype='split' ">
          <xsl:call-template name="tpl_step_joined_split">
            <xsl:with-param name="visible" select="$childvisible"/>
          </xsl:call-template>
        </xsl:when>
		  <xsl:when test="$routetype='consult' and $splittype='' ">
			  <xsl:call-template name="tpl_step_joined">
				  <xsl:with-param name="visible" select="$childvisible"/>
			  </xsl:call-template>
		  </xsl:when>
        ...
        <xsl:when test="$routetype='assist' and $splittype='split' ">
          <xsl:call-template name="tpl_step_joined_split">
            <xsl:with-param name="visible" select="$childvisible"/>
          </xsl:call-template>
        </xsl:when>
		  <xsl:when test="$routetype='assist' and $splittype='' ">
			  <xsl:call-template name="tpl_step_joined">
				  <xsl:with-param name="visible" select="$childvisible"/>
			  </xsl:call-template>
		  </xsl:when>		  
	...
	<xsl:template name="tpl_step_joined_split" match="*">
		<xsl:param name="visible"/>
		<xsl:for-each select="*/item">
			<step>
				<xsl:attribute name="unittype">
				...
				<xsl:attribute name="routetype">
					<xsl:value-of select="$routetype"/>
				</xsl:attribute>
				<xsl:if test="$allottype!=''">
					<xsl:attribute name="allottype">
						<xsl:value-of select="$allottype"/>
					</xsl:attribute>
				</xsl:if>
	...
	<xsl:template name="tpl_user_item" match="*">
		...
		<ou>
			...
			<xsl:attribute name="company"><xsl:value-of select="ETNM"/></xsl:attribute>
			<person>
				<xsl:attribute name="code"><xsl:value-of select="AN"/></xsl:attribute>
				<xsl:attribute name="name"><xsl:value-of select="DN"/></xsl:attribute>
				...
				<xsl:attribute name="ouname">
					<xsl:if test="$ounametype!='0'">
						<xsl:value-of select="ETNM"/>-
					</xsl:if>
					<xsl:value-of select="DP"/>
				</xsl:attribute>
				<xsl:attribute name="sipaddress"><xsl:value-of select="SIP"/></xsl:attribute>
        <xsl:attribute name="stampRights"><xsl:value-of select="$stampRights"/></xsl:attribute> <!-- 인장권자여부 (2012-12-17 HIW) -->
				<xsl:if test="$routetype!='notify' and $routetype!='ccinfo'">
					<xsl:call-template name="tpl_taskinfo">
						<xsl:with-param name="visible" select="$visible"/>
					</xsl:call-template>
	...
	<xsl:template name="tpl_dist_item" match="*">
		...
		<group>
			...
			<xsl:attribute name="name"><xsl:value-of select="DN"/></xsl:attribute>
			<xsl:attribute name="company"><xsl:value-of select="ETNM"/></xsl:attribute>
			<xsl:attribute name="comname"><xsl:value-of select="RGNM"/></xsl:attribute>
	...
	<xsl:template name="tpl_taskinfo" match="*">
        ...
        <xsl:when test="$kindext='confirm'">confirm</xsl:when>
        <xsl:when test="$kindext='reference'">reference</xsl:when>
